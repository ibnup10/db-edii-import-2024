    ...
    # Menambahkan kolom "DOKUMEN" setelah "KODE DOKUMEN"
        if "KODE DOKUMEN" in df.columns:
            df.insert(df.columns.get_loc("KODE DOKUMEN") + 1, 'DOKUMEN',
              df["KODE DOKUMEN"].map(lambda x: "E-COO" if x == "860" else 
                                               "COO" if x == "861" else 
                                               "RCEP" if x == "864" else ""))
...
columns_to_remove = ["KODE SATUAN", "JUMLAH SATUAN", "KODE KEMASAN", "JUMLAH KEMASAN", "Jumlah Nilai CIF", "SERI BARANG","KODE FASILITAS"]
...
def search_data(data, keyword, column=None):
    ...
    if column == "HS":
        ...
        if keyword == "e-coo":
            ...
            return data[data["DOKUMEN"] == "RCEP"].drop_duplicates()
...
def main():
    ...
    if data is not None:
        ...
        if selected_column == "DOKUMEN":
            keyword = st.selectbox("Pilih Jenis Dokumen:", ["E-COO", "COO", "RCEP", "TANPA FASILITAS"])
        else:
